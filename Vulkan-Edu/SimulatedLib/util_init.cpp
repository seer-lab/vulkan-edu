...
void vulkanInit(struct sample_info& info) {
    if (!glfwInit()) {
    ...
    for (int i = 0; i < ext_count; i++) {
        info.instance_extension_names.push_back(extName[i]);
        cout << info.instance_extension_names[i] << endl;
    }
}
...
void init_window(struct sample_info& info) {
    ...
    //    exit(1);
    //}
    //SetWindowLongPtr(info.window, GWLP_USERDATA, (LONG_PTR)&infb o);

    glfwDefaultWindowHints();
    glfwWindowHint(GLFW_CLIENT_API, GLFW_NO_API);
    info.windows = glfwCreateWindow(info.width, info.height, (const char*)info.name, NULL, NULL);

    if (glfwCreateWindowSurface(info.inst, info.windows, nullptr, &info.surface) != VK_SUCCESS) {
...
void init_swapchain_extension(struct sample_info& info) {
    ...
    createInfo.hinstance = GetModuleHandle(NULL);//info.connection;
        createInfo.hwnd = glfwGetWin32Window(info.windows);//info.windows;
